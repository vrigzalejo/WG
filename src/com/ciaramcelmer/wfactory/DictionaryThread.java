...
public class DictionaryThread implements Runnable {
	...
	public static final int MESSAGE_HAVE_NINELETTER_ON_PROGRESS = 11;
	public static final int MESSAGE_GET_MATCHING_WORDS_ON_PROGRESS = 12;
	public static final int MESSAGE_HAVE_MATCHING_WORDS_ON_PROGRESS = 13;
	...
	static ArrayList<NineLetterWord> nineLetterWords;
	
	// List of valid words for current Nine letter
	public ArrayList<String> validWords;
	
	public static NineLetterWord currentNineLetter;
	...
	public void run() {
		...
		// Handler must be created in the thread (e.g.: inside the run() method)
				...
				switch(msg.what) {
					...
					case MESSAGE_GET_NINELETTER : {
						...
						do {
							...
							if (attempts++ > 100)
								break;
						} while (NineLetterWord.shuffleWithRange(currentNineLetter, minSize,
								maxSize) == false);
						// Send message back saying we have the word
						Message message = Message.obtain();
						message.what = MESSAGE_HAVE_NINELETTER;
					...
					case MESSAGE_GET_MATCHING_WORDS_ON_PROGRESS : {
						// Find words matching current nine letter (shuffled)
						validWords = new ArrayList<String>();
						getAllMatchingWords();
						...
						// Send notification back to main thread
						Message message = Message.obtain();
						message.what = MESSAGE_HAVE_MATCHING_WORDS_ON_PROGRESS;
						MainActivity.currentInstance.newWordReadyHandler.sendMessage(message);
						break;
					}
	...
	private void getNineLetterWords(int dictionary) {
		...
		Log.d("Word Factory", "Read all 9 letter words.");
	}
	
	// From "currentNineLetterWord", with an unknown word 
	// (e.g.: only the scrambled word), attempt to find the word. 
	// Returns boolean, whether or not a word was found.
	...
	private void getAllMatchingWords() {
		...
		// First initialize an array for letters we've already searched
		char[] searchedLetters = new char[9];
		for (int i = 0 ; i < 9 ; i++)
			searchedLetters[i] = '\0';
		
		// Now fo thru nineletters and see if we've
		...
		for (int i = 0 ; i < 9 ; i++) {
			...
			if (searchedLetterAlready)
				continue;
			// New letter, find some words.
			// Only split common words out, as that is the most time consuming.
				switch(letter) {
					case 'A' :
			...
			private void getMatchingWords(int dictionary) {
				...
				Log.d("Word Factory", "Found matches, " + validWords.size() + " words");
			}
			...
			private void getDictionary() {
				String dict = PreferenceManager.getDefaultSharedPreferences(MainActivity.currentInstance).getString("dictpref", "1");
				if (dict.equals("2")) {
					Toast.makeText(MainActivity.currentInstance, 
							"Select your dictionary in Options, defaulting to US.",
